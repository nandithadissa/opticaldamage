...
def plot2(filename):
    ...
    def gaussian_fit(x,y):
        ...
        #plt.scatter(x[1:],zfit[1:])
        x,y_fit=gaussian(x,A,B,W)
        plt.plot(x-0.075,y_fit/np.max(y_fit),'-.')
        return
...
plot1("beamwidth-nodiffuser-PRF-10KHz_1_beamwidth.txt")
plot2("beamwidth-diffuser-PRF-10KHz_1_beamwidth.txt")
plt.xlim(-1,1)
plt.show()
